...
(function (global) { // IIFE
    ...
    AjaxUtils.SendGetRequest = function (myURL, ResponceHandler, isJson) {
        ...
        request.onreadystatechange = function () { // The onreadystatechange property defines a function to be executed when the readyState changes.
            if (this.readyState === 4 && this.status === 200) {
                ResponceHandler(request, isJson); // Call the function which will process the response from the server, which is located in the above created XMLHttpRequest.
            }
        };
